    ...
    <main className="flex-1 p-8 md:p-12 max-w-7xl mx-auto w-full">
      ...
      <div className="bg-white rounded-[20px] shadow-[0_12px_40px_rgba(24,28,30,0.06)] overflow-hidden flex flex-col font-body">
        ...
        <div className="bg-slate-50 border-b border-surface-container/50 px-8 py-5 grid grid-cols-12 gap-4 text-xs font-bold text-slate-500 uppercase tracking-widest">
          ...
          <div className="col-span-5">Logistics</div>
          ...
          <div className="flex flex-col">
            {bookings.map((b) => (
              <div key={b.id} className="grid grid-cols-12 gap-4 px-8 py-6 items-center even:bg-white odd:bg-[#f7fafc] hover:bg-slate-100/60 transition-colors">
                ...
                <div className="col-span-2 flex flex-col text-sm text-slate-700">
                  <div className="flex items-center font-medium">
                    {new Date(b.moveDate).toLocaleDateString()}
                  </div>
                  <span className="text-xs text-slate-500 font-semibold">{b.moveTime}</span>
                </div>

                {/* Logistics Requirements */}
                <div className="col-span-5 flex flex-col truncate pr-4 text-sm">
                  ...
                  <div className="flex flex-col items-start text-xs text-slate-500 mt-1">
                    <div className="flex items-center">
                      <span className="text-[8px] mr-1">FROM:</span>
                      <span className="truncate" title={`${b.originLabel || b.pickupLocation || '—'}`}>
                        {b.originLabel || b.pickupLocation || '—'}
                      </span>
                    </div>

                    <div className="flex items-center">
                      <span className="text-[8px] mr-1">TO:</span>
                      <span className="truncate" title={`${b.destLabel || b.dropoffLocation || '—'}`}>
                        {b.destLabel || b.dropoffLocation || '—'}
                      </span>
                    </div>
                  </div>

                  {(b.distanceKM || b.estPrice) && (
                    <div className="flex items-center text-[10px] text-slate-400 font-semibold tracking-tight">
                      <span className="text-[8px] mr-1">DIST:</span>
                      {b.distanceKM && <span>{(b.distanceKM || 0).toFixed(1)} km</span>}
                    </div>
                  )}
                </div>
                ...
                {/* Price */}
                <div className="col-span-1 flex flex-col truncate pr-4 text-sm">
                  {b.estPrice && (
                    <div className="font-semibold text-slate-900">
                      {b.estPrice && <span>{(b.estPrice || 0).toLocaleString()}฿</span>}
                    </div>
                  )}
                </div>
                ...
                <div className="col-span-1 flex justify-end relative group">
                  ...
                  >
                    <option value="pending" className="font-semibold text-slate-900 py-1">Pending</option>
