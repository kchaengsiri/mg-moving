...
export default async function Home({ params }: { params: Promise<{ lang: string }> }) {
  ...
  return (
    <main className="min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 grid md:grid-cols-2 gap-12 items-center">
          <div className="space-y-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              {/* Primary Button */}
              <Link href={`/${lang}/book`} className="bg-tertiary-fixed text-on-tertiary-fixed px-8 py-4 rounded-md font-bold font-body text-base hover:opacity-90 transition-all duration-300 active:scale-95 gold-glow shadow-lg shadow-black/10 text-center">
      ...
      {/* Our Work — Portfolio Section (only rendered when items exist) */}
      {portfolioItems.length > 0 && (
        <section id="our-work" className="py-24 bg-surface-container-low">
          <div className="max-w-7xl mx-auto px-6">
            <div className="mb-14">
              <span className="text-on-tertiary-container font-label font-bold tracking-[0.2em] uppercase text-xs mb-2 block">{t.ourWork.label}</span>
              <h2 className="font-headline text-4xl md:text-5xl font-extrabold text-primary tracking-tight">{t.ourWork.title}</h2>
              <p className="text-secondary font-body mt-4 max-w-xl">{t.ourWork.desc}</p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
              {portfolioItems.map((item) => (
                <div key={item.id} className="group bg-surface-container-lowest rounded-xl overflow-hidden shadow-[0_4px_20px_rgba(24,28,30,0.03)] hover:shadow-[0_12px_40px_rgba(24,28,30,0.08)] transition-all duration-300 cursor-pointer">
                  ...
                  <div className="p-6">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 bg-primary/10 text-primary rounded-full">{item.category}</span>
                    </div>
                    <h3 className="font-headline font-bold text-lg text-primary mb-2">{item.title}</h3>
                    <p className="text-sm text-secondary font-body line-clamp-2">{item.description}</p>
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* AIEO Semantic JSON-LD Injection */}
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "address": {
              "@type": "PostalAddress",
              "streetAddress": "Si Sunthon Office",
              "addressLocality": "Thalang District, Phuket",
              "postalCode": "83110",
              "addressCountry": "TH"
            }
          })
